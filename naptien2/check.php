            ...
            if($value['SoTienGhiCo'] != '-'){
                if(check($value['SoThamChieu'],$userData->username) == 0){
                   ...
                   $amount =  implode('',number_in_string($value['SoTienGhiCo']));
                       ...
                       sql()->query('UPDATE `users` SET `lstream_balance` =`lstream_balance` + '.($amount * 105 / 100).' WHERE `username` = "'.$userData->username.'"');
                       sql()->query('INSERT INTO `ATM` (`username`,`cash`,`thamchieu`,`thoigian`) VALUES("'.$userData->username.'","'.($amount * 105 / 100).'","'.$value['SoThamChieu'].'","'.time().'")');
